                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className={`text-sm font-black truncate leading-tight ${isCurrent ? 'text-white' : 'text-slate-800'}`}>
                          {slide.type === 'candidate' ? slide.content.name : (slide.title || slide.id.replace(/-/g, ' ').toUpperCase())}
                        </div>
                        <div className={`text-[10px] font-bold truncate opacity-70 ${isCurrent ? 'text-indigo-100' : 'text-slate-500'}`}>
                          {slide.type === 'candidate' ? `${slide.content.age} años` : (slide.subtitle || slide.type.replace(/-/g, ' ').toUpperCase())}
                        </div>
                      </div>
                      {isCurrent && (
